...
async fn main_loop() -> Result<()> {
    ...
    let (key, chain) = ssl::get_key_and_cert();
    let cert = Identity::from_der(key, chain)?;
    
    let tls_acceptor = tokio_native_tls::TlsAcceptor::from(
        native_tls::TlsAcceptor::builder(cert)
            .request_alpns(ssl::ALPN_LIST)
            .build()?
    );
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let res = http.serve_connection(tls_stream, service_fn(handle_request)).await;
